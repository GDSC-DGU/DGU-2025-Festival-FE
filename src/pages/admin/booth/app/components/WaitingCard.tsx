              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                ...
                {elapsed && (
                  <S.ElapsedText style={{ marginLeft: 16 }}>
                    <img src={Time} alt="경과 시간" style={{ width: 10, height: 10, marginRight: -3, verticalAlign: 'middle' }} />
                    {elapsed}
                  </S.ElapsedText>
                )}
              </div>
              <S.VisitButton onClick={(e) => { e.stopPropagation(); handleClick('visit'); }}>입장 완료</S.VisitButton>
              {showDeleteButton && (
                <S.DeleteButton onClick={(e) => { e.stopPropagation(); handleClick('delete'); }}>대기 취소</S.DeleteButton>
              )}
            </>
          ) : (
            <>
              <S.CallButton onClick={(e) => { e.stopPropagation(); handleClick('call'); }}>호출</S.CallButton>
              <S.DeleteButton onClick={(e) => { e.stopPropagation(); handleClick('delete'); }}>대기 취소</S.DeleteButton>
            </>
          )}
        </S.ButtonGroup>
