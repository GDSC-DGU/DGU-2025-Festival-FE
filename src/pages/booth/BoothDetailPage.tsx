...
import {
  Container, MapWrapper, Card, Header, Info, BoothName,
  BoothIntro, Like, LikeCount, ReserveButton, ScrollWrapper,
  ContentContainer, SubContainer, Title, ImageScrollContainer,
  ImageItem, ImageScrollWrapper,
...
export default function BoothDetailPage() {
  ...
  const [waitingClosedModalMessage, setWaitingClosedModalMessage] = useState("");
  ...
  const loopedBooths = useMemo(() => [...relatedBooths, ...relatedBooths], [relatedBooths]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (autoScrollIndex.current >= loopedBooths.length - relatedBooths.length) {
        ...
        scrollEl.scrollTo({
          left: jumpTarget.offsetLeft - scrollEl.offsetWidth / 2 + jumpTarget.offsetWidth / 2,
          behavior: "auto",
        });
        return;
      }
      ...
      scrollEl.scrollTo({
        left: target.offsetLeft - scrollEl.offsetWidth / 2 + target.offsetWidth / 2,
        behavior: "smooth",
      });
    }, 3000);
      ...
      <ContentContainer>
        ...
        <Card>
                ...
                <ReserveButton
                  onClick={() => {
                    if (pubStatus === "AVAILABLE") {
                      setWaitingClosedModalMessage("현재 바로 입장 가능합니다.");
                      setShowWaitingClosedModal(true);
                    } else if (pubStatus === "PREPARING") {
                      setWaitingClosedModalMessage("아직 부스가 시작되지 않았어요. 조금만 더 기다려주세요!");
                      setShowWaitingClosedModal(true);
          ...
          {booth.images.length > 0 && (
            <ImageScrollWrapper>
              <ImageScrollContainer ref={imageScrollRef} onScroll={handleScroll}>
